...
## 总结
* 在本章中，您了解到Amazon VPC是Amazon EC2的网络层，它允许您在云中创建自己的私有虚拟网络。您可以提供自己的AWS逻辑隔离部分，类似于设计和实现在物理数据中心中操作的独立网络。
...
* 子网只能是公用、专用或VPN。公共子网是关联的路由表将子网的流量定向到Amazon VPC的IGW的子网。私有子网是关联的路由表不将子网的流量定向到Amazon VPC的IGW的子网。只有VPN的子网是这样一个子网，其中关联的路由表将子网的流量定向到Amazon VPC的VPG，并且没有到IGW的路由。无论子网的类型如何，子网的内部IP地址范围始终是私有的（在Internet上是不可路由的）。

* 路由表是Amazon VPC中的一个逻辑结构，它包含一组应用于子网并用于确定网络流量指向何处的规则（称为路由）。路由表的路由允许Amazon VPC内不同子网中的Amazon EC2实例相互通信。您可以修改路由表并添加自己的自定义路由。您还可以使用路由表指定哪些子网是公共的（通过将Internet流量定向到IGW）哪些子网是私有的（通过没有将流量定向到IGW的路由）。IGW是一个水平扩展的、冗余的、高可用的Amazon VPC组件，它允许Amazon VPC中的实例与Internet之间进行通信。IGWs是完全冗余的，没有带宽限制。IGW在Amazon VPC路由表中为Internet可路由流量提供一个目标，并为已分配公共IP地址的实例执行网络地址转换。

* Amazon VPC的DHCP option sets元素允许您将Amazon EC2主机名分配定向到自己的资源。为了将自己的域名分配给实例，需要创建一个自定义的DHCP选项集并将其分配给Amazon VPC。

* EIP地址是池中的静态公用IP地址，用于可以分配给帐户（从池中提取）和释放（返回池）的区域。EIP允许您维护一组保持固定的IP地址，而底层基础设施可能会随着时间而改变。

* Amazon VPC端点允许您在Amazon VPC和另一个AWS服务之间创建私有连接，而无需通过Internet或NAT实例、VPN连接或AWS直接连接进行访问。可以为单个服务创建多个终结点，还可以使用不同的路由表从不同的子网对同一服务强制实施不同的访问策略。

* Amazon VPC对等连接是两个Amazon VPC之间的一种网络连接，它使任一Amazon VPC中的实例能够彼此通信，就好像它们在同一个网络中一样。您可以在自己的Amazon VPC之间创建一个Amazon VPC对等连接，也可以在单个区域内的另一个AWS帐户中创建一个Amazon VPC对等连接。对等连接既不是网关也不是VPN连接，并且不会引入通信的单点故障。

* 安全组是一个虚拟有状态防火墙，它控制到Amazon EC2实例的入站和出站流量。当您第一次在Amazon VPC中启动Amazon EC2实例时，必须指定与之关联的安全组。AWS提供了一个供您使用的默认安全组，其中包含允许与该安全组关联的所有实例相互通信并允许所有出站流量的规则。您可以更改默认安全组的规则，但不能删除默认安全组。

* 网络ACL是另一个安全层，在子网级别充当无状态防火墙。Amazon vpc是用一个可修改的默认网络ACL创建的，该ACL与允许所有入站和出站流量的每个子网相关联。如果要创建自定义网络ACL，其初始配置将拒绝所有入站和出站流量，直到您创建一个声明为其他的规则。

* NAT实例是一个客户管理的实例，它被设计为接受来自私有子网内实例的流量，将源IP地址转换为NAT实例的公共IP地址，并将流量转发到IGW。此外，NAT实例维护转发流量的状态，以便将响应流量从Internet返回到私有子网中的适当实例。

* NAT网关是AWS管理的服务，设计用于接受来自私有子网内实例的流量，将源IP地址转换为NAT网关的公共IP地址，并将流量转发到IGW。此外，NAT网关维护转发流量的状态，以便将响应流量从Internet返回到私有子网中的适当实例。

* VPG是两个网络之间VPN连接的AWS端的VPN集中器。CGW是VPN连接的客户端的物理设备或软件应用程序。在创建了Amazon VPC的这两个元素之后，最后一步是创建VPN隧道。VPN隧道是在从VPN连接的客户端。

## 考试基础
### 理解什么是VPC，他的核心组件喝可选组件
* Amazon VPC是AWS云中逻辑隔离的网络。Amazon VPC由以下核心元素组成：子网（仅限公共、私有和VPN）、路由表、DHCP选项集、安全组和网络acl。可选元素包括IGW、EIP地址、端点、对等连接、NAT实例、VPG、CGWs，和VPN连接。

### 理解子网的用途
* 子网是Amazon VPC IP地址范围的一部分，您可以在其中放置独立资源组。子网由CIDR块定义，例如10.0.1.0/24和10.0.2.0/24，并包含在可用性区域中。

### 识别public子网，private子网，VPN-only子网之间的不同
* 如果子网的流量路由到IGW，则该子网称为公用子网。如果子网没有到IGW的路由，则该子网称为私有子网。如果子网没有到IGW的路由，但其流量路由到VPG，则该子网称为VPN-only的子网 

### 理解路由表的用途
* 路由表是一组规则（称为路由），用于确定网络流量的流向。路由表允许不同子网中的Amazon EC2实例相互通信（在同一Amazon VPC内）。Amazon VPC路由器还允许子网、igw和vpg相互通信。

### 理解IGW的用途
* IGW是一个水平扩展的、冗余的、高可用的Amazon VPC组件，它允许Amazon VPC中的实例与Internet之间进行通信。IGWs是完全冗余的，没有带宽限制。IGW在Amazon VPC路由表中为Internet可路由流量提供目标，并为已分配公共IP地址的实例执行网络地址转换。

### 理解DHCP选项集为Amazon VPC提供了什么
* Amazon VPC的DHCP option sets元素允许您将Amazon EC2主机名分配定向到自己的资源。您可以为Amazon VPC中的实例指定域名，并标识自定义DNS服务器、NTP服务器和NetBIOS服务器的IP地址。

### 知道VPC的public IP地址与EIP地址之间的不同
* 公用IP地址是AWS拥有的IP，可以自动分配给子网中启动的实例。EIP地址是AWS拥有的公用IP地址，您可以分配给您的帐户，并根据需要分配给实例或网络接口。

### 理解Endpoint为Amazon VPC提供了什么
* Amazon VPC端点允许您在Amazon VPC和另一个AWS服务之间创建私有连接，而无需通过Internet或NAT实例、VPN连接或AWS直接连接进行访问。终结点仅支持Region内的服务。

### 理解Amazon VPC peering
* 一个Amazon VPC对等连接是两个Amazon VPC之间的网络连接，它使任何一个Amazon VPC中的实例能够彼此通信，就好像它们在同一个网络中一样。对等连接是通过请求/接受协议创建的。不支持传递对等，对等仅在同一区域内的Amazon VPC之间可用

### 知道安全组和网络ACL的不同
* 安全组在实例级别应用。您可以在多个子网中有多个实例，这些子网是同一安全组的成员。安全组是有状态的，这意味着不管出站规则如何，都自动允许返回通信量。网络ACL应用于子网级别，并且通信量是无状态的。您需要允许网络ACL上的入站和出站流量，以便子网中的Amazon EC2实例能够通过特定协议进行通信。

### 理解NAT为Amazon VPC提供了什么
* NAT实例或NAT网关允许私有子网中的实例启动到Internet的出站通信。例如，这允许出站Internet通信下载修补程序和更新，但阻止实例接收Internet上某人发起的入站通信。

### 理解建立从网络到Amazon VPC的VPN连接所需的组件
* VPG是两个网络之间VPN连接的AWS端的VPN集中器。CGW表示VPN连接的客户端的物理设备或软件应用程序。VPN连接必须从CGW端启动，并且连接由两个IPSec隧道组成。

## 练习
* 熟悉Amazon VPC的最好方法是构建自己的自定义Amazon VPC，然后将Amazon EC2实例部署到其中，这就是本节将要做的。您应该重复这些练习，直到您可以自信地创建和停用Amazon VPCs。有关完成这些练习的帮助，请参阅位于http://aws.Amazon.com/documentation/VPC/的Amazon VPC用户指南。

### 练习 4.1 
#### 创建一个客户Amazon VPC

* 1. 作为管理员或者有权限的用户登录AWS管理控制台；
* 2. 选在Amazon VPC ICON启动Amazon VPC Dashboard；
* 3. 创建一个Amazon VPC，CIDR块等于192.168.0.0/16，名称标签为：My First VPC，默认租赁。

你已经创建了你的第一个客户VPC

### 练习 4.2
#### 为你的客户Amazon VPC创建两个子网
* 1. 创建一个子网，CIDR块等于192.168.1.0/24，并且名称标签为：My First Public Subnet。根据练习4.1在Amazon VPC中创建子网，并指定子网的可用区域（例如，US-East-1a）。
* 2。创建一个子网，CIDR块等于192.168.2.0/24，并且名称标签为：My First Private Subnet。根据练习4.1在Amazon VPC中创建子网，并为子网指定一个不同于先前指定的可用区域（例如，US-East-1b）。

* 现在您已经创建了两个新的子网，每个子网都位于其自己的可用性区域中。记住一个子网等于一个可用区域是很重要的。不能跨多个可用性区域拉伸子网。

### 练习 4.3
#### 将您的自定义Amazon VPC连接到Internet并建立路由
以获得此练习的帮助，请参阅Amazon EC2密钥对文档：http://docs.aws.Amazon.com/aws EC2/latest/UserGuide/EC2-key-pairs.html以获取此练习的其他帮助，请参阅NAT实例文档：http://docs.aws.amazon.com/amazonpvc/latest/UserGuide/VPC_NAT_Instance.html#NATInstance 

* 1。在与自定义Amazon VPC相同的区域中创建Amazon EC2密钥对。
* 2。创建一个带有我的第一个IGW名称标签的IGW，并将其附加到您的自定义Amazon VPC。
* 3. 将路由添加到自定义Amazon VPC的主路由表中，该表将互联网流量（0.0.0.0/0）定向到IGW。
* 4. 创建一个NAT网关，将其放置在自定义Amazon VPC的公共子网中，并为其分配一个EIP。
* 5. 使用我的第一个私有路由表的名称标签创建一个新的路由表，并将其放置在您的自定义Amazon VPC中。向其添加一个将Internet流量（0.0.0.0/0）定向到NAT网关的路由，并将其与专用子网关联。

* 现在，您已经为Amazon VPC内的资源创建了到Internet的连接。您建立了将Internet流量定向到IGW的路由规则，而不考虑原始子网。  

### 练习 4.4
#### 启动一个Amazon EC2实例并测试与Internet的连接
* 1. 将t2.micro Amazon Linux AMI作为Amazon EC2实例启动到自定义Amazon VPC的公共子网中，给它一个My First Public Instance 的名称标签，并选择新创建的密钥对以安全访问该实例。
* 2. 使用新创建的密钥对通过SSH安全地访问公共子网中的Amazon EC2实例。
* 3. 通过执行以下命令对操作系统实例库执行更新：
```
  sudo yum update-y  
```
* 4. 您应该看到显示实例从Internet下载并安装软件的输出。

* 您现在已经在公共子网中设置了Amazon EC2实例。您可以将补丁应用到公共子网中的Amazon EC2实例，并演示了与Internet的连接。
