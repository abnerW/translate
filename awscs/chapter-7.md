...
### 主键
* 创建表的时候，除了表名还要指定主键。与关系数据库一样，表中的主键唯一地表示每个item。一个主键准确地指向一个item。Amazon DynamoDB提供了两种类型的主键，表创建之后，此配置不能修改。

#### Partition Key
* 由一个属性和一个分区键（或者hash key）构成；Amazon DynamoDB 在主键属性上构建一个无序的hash索引。

#### Partition 和 Sort key
...
* 表中的item是通过分区键和排序键的值的组合唯一标识

* 表中的两个items可能有一样的分区键值，但是他们的排序键值一定不同

* 另外，每个主键属性必须被定义为：string，number，或者binary类型。Amazon DynamoDB 通过分区键将请求分发到合适的分区。

* 备注
```
 如果使用同样的主键执行的量的读写（每秒）；Amazon DynamoDB的计算能力无法完全发挥出来。
 最佳实践：是通过在整个分区键范围内分发请求来最大化吞吐量
```

### 容量调配

* 当创建一个表时，需要规定一定的读写能力来处理期望的负载。Amazon DynamoDB会基于初始配置，提供合适的基础配置，以满足持续、低延迟相应时间的需求。这些都通过读写能力单元数进行衡量，这个值可以在随后通过UpdateTable动作进行调大或者调小。

* 针对Amazon DynamoDB表的每个操作都将消耗一些已配置的容量单元。容量单位的具体消耗量在很大程度上取决于item的规模，但也取决于其他因素。对于读取操作，所消耗的容量也取决于在请求中选择的读取一致性。在本章后面阅读更多关于最终一致性和强一致性的内容。

* 例如：一个没有本地二级索引的表，从中读一个4KB或者更小的item，将会消耗一个容量单元。类似，向表中写一个1KB或者更小的item，将会消耗一个容量单元。如果从表中读110KB，将会消耗28个容量单元。对于强一致性的读操作来说，将会消耗两倍的容量单元，本例中将会消耗：56个容量单元。

* 可以通过CloudWatch监控容量单元的使用情况，并作出是否扩容的决定。有丰富的指标集，包括：ConsumedReadCapacityUnits，ConosumedWriteCapacityUnits。如果再某段时间内，请求使用的容量单元超过了设定值，请求将会卡死，并再稍后时间重试。您可以使用Amazon CloudWatch监视ThrottledRequests度量并发出警报，以通知您使用模式的更改。

### 辅助索引

* 当你创建一个有分区键和排序键的表时，可以在这个表上创建一个或者多个辅助索引。辅助索引可以使你在查询表中的数据时不用主键，用这个可以来代替。Amazon DynamoDB 提供两种索引：

#### Global Secondary Index

* 全局辅助索引有分区和排序键组成，不同于表上已经存在的分区和排序键；你可以在任何时间创建或者删除这种索引。

#### Local Secondary Index

* 本地辅助索引有分区和排序键组成，分区键和表上主键的分区键一样，排序键则不同。只能在创建表的时候创建一个这种索引。

* 辅助索引允许您高效地搜索大型表，并避免昂贵的扫描操作来查找具有特定属性的项。这些索引允许您支持不同的查询访问模式和用例，而不仅仅是使用主键。虽然一个表只能有一个本地辅助索引，但可以有多个全局辅助索引。

* 当表中的item被修改时，Amazon DynamoDB会更新每个辅助索引。这个操作消耗写容量单元。对于本地辅助索引，项更新将消耗主表中的写容量单位，而全局辅助索引则保持其自己独立于表的配置吞吐量设置。

### 读写数据

* 创建具有主键和索引的表后，可以开始向该表写入和读取项。Amazon DynamoDB提供了多个操作，允许您创建、更新和删除单个项目。Amazon DynamoDB还提供了多个查询选项，允许您搜索表或索引，或检索特定项或一批项。

#### 写items
* Amazon DynamoDB提供了三个主要的：创建，更新和删除items的API：PutItem，UpdateItem以及DeleteItem。

* 使用PutItem可以创建：带有一个或者多个属性的Item；如果主键存在，PutItem的调用将会更新存在的项，PutItem必须有：表名和主键，其它属性是可选的。

* UpdateItem 操作将基于主键查找现有项并替换属性。此操作可用于仅更新单个属性并保持其他属性不变。UpdateItem还可以用于创建不存在的项。最后，可以使用 deleteeitem 并指定一个特定的主键从表中删除一个项。

* UpdateItem 操作还支持原子计数器。原子计数器允许您递增和递减一个值，并保证在多个并发请求之间保持一致。例如，许多客户端可以同时更新用于跟踪移动游戏整体得分的计数器属性。

* 这三个操作还支持允许您在应用操作之前执行验证的条件表达式。例如，可以对PutItem应用条件表达式，该表达式检查在创建项之前是否满足某些条件。这对于防止意外覆盖或强制执行某种类型的业务逻辑检查非常有用

#### 读items

* 创建项后，可以通过调用GetItem操作直接查找或使用Query或Scan操作进行搜索来检索该项。GetItem允许您基于项的主键检索项。默认情况下，项目的所有属性都会返回，您可以选择单个属性来筛选结果。

* 如果主键由分区键组成，则需要指定整个分区键来检索项。如果主键是分区键和排序键的组合，那么GetItem也需要分区键和排序键。每次调用GetItem都会根据项的大小和所选的一致性选项消耗读容量单位。

* 默认情况下，GetItem操作执行最终一致的读取。您可以选择请求强一致性读取；这将消耗额外的读取容量单位，但它将返回项目的最新版本

### 最终一致性
* 当从Amazon DynamoDB读取项目时，操作可以是最终一致的，也可以是强一致的。Amazon DynamoDB是一个分布式系统，它在AWS区域中存储一个项目的多个副本，以提供高可用性和更高的耐用性。当一个条目在amazondynaminodb中更新时，它开始跨多个服务器进行复制。因为Amazon DynamoDB是一个分布式系统，所以复制可能需要一些时间才能完成。因此，我们将数据称为最终一致的，这意味着写操作之后的读请求可能不会显示最新的更改。在某些情况下，应用程序需要保证数据是最新的，而amazondynaminodb提供了一个强一致性读取选项。

#### 最终一致性读
* 读取数据时，响应可能不会反映最近完成的写入操作的结果。响应可能包含一些过时的数据。数据的所有副本之间的一致性通常在一秒钟内达到；如果在短时间后重复读取请求，则响应将返回最新的数据。

#### 强一致性读
* 当您发出一个强一致的读取请求时，Amazon DynamoDB返回一个带有最新数据的响应，该数据反映了之前所有相关写操作的更新，Amazon DynamoDB返回了一个成功的响应。在网络延迟或中断的情况下，强一致性读取可能不太可用。通过在请求中指定可选参数，可以请求强一致的读取结果。

### 批量操作

* Amazon DynamoDB 提供了许多为大批量Items而设计的操作，包括：BatchGetItem和BatchWriteItem。用BatchWriteItem行为，可以在一个操作里最多为**25**个Items执行创建或者更新。在处理大量的Items时，这种操作可以最小化每一个单独调用的开销。

#### 搜索Items
* Amazon DynamoDB也为你提供了了两个操作：query和scan。用于搜索一个表或者索引。一个QUERY操作是主要的搜索操作，可以用用于在表上发现items，或者仅仅使用了主键属性值的二级索引。每次QUERY需要一个分区key属性名和一个distinct值用于搜索。您可以选择提供排序关键字值并使用比较运算符来优化搜索结果。结果将按主键自动排序，并限制为**1MB**

* 与QUERY相反，SCAN操作将读取表或辅助索引中的每个项。默认情况下，SCAN 操作返回表或索引中每个项的所有数据属性。每个请求最多可以返回**1MB**的数据。可以使用表达式筛选出项，但这可能是一个资源密集型操作。如果查询或扫描的结果集超过1MB，则可以以1MB为增量对结果进行分页。

* 备注
对于大部分的操作，执行QUERY代替SCAN操作是最有效的选项。执行SCAN操作将会对表或者二级索引进行全量扫描，过滤值后提供想要的结果。尽可能使用QUERY操作，避免对大表或者在仅仅有少量Items的扫描


### 扩展和分区
* Amazon DyanmoDB是一个完全托管的服务，它抽象了构建和扩展NoSQL集群所涉及的大部分复杂性。您可以创建可以扩展的表，以容纳几乎无限个具有一致的低延迟性能的Items。Amazon DynamoDB表可以通过使用分区来水平扩展，以满足应用程序的存储和性能要求。每个单独的分区代表一个计算和存储容量单位。一个设计良好的应用程序将考虑表的分区结构，以均匀地分配读写事务，并在低延迟下实现高事务速率

* Amazon DynamoDB跨多个分区存储单个表的项，如图7.4所示。 Amazon DynamoDB根据分区键决定将项目存储在哪个分区中。分区键用于在所有可用分区之间分发新项，具有相同分区键的项将存储在同一分区上。

* 图7.4 表分区

* 随着表中项目数的增加，可以通过拆分现有分区来添加其他分区。为表配置的已配置吞吐量也在分区之间平均分配。分配给分区的已配置吞吐量完全专用于该分区，并且分区之间不共享已配置吞吐量。


* 创建表时，Amazon DynamoDB根据所需的读写容量配置表的分区。一个单独的分区可以容纳大约**10GB**的数据，最多支持**3000**个读容量单元或**1000**个写容量单元。对于没有完全使用其供应容量的分区，Amazon DynamoDB提供了一些突发容量来处理流量峰值。一部分未使用的容量将被保留以处理短时间的突发事件。


* 随着存储或容量需求的变化，Amazon DynamoDB可以分割一个分区以容纳更多的数据或更高的配置请求速率。但是，分割分区后，不能将其合并回一起。在计划临时增加供应容量，然后再次降低供应容量时，请记住这一点。随着每个附加分区的添加，它在已配置容量中所占的份额将减少。


* 要实现为表提供的全部请求吞吐量，请使工作负载均匀分布在分区键值上。跨分区的分发请求键值跨分区分发请求。例如，如果一个表配置了10000个读容量单元，但所有通信量都达到一个分区键，那么您将无法获得超过一个分区所能支持的3000个最大读容量单元

* 备注
为了最大化Amazon DynamoDB的吞吐量，创建具有大量不同值的分区键的表，并确保这些值的请求相当一致。添加可以计算或散列的随机元素是改进分区分布的一种常用技术

### 安全
* Amazon DynamoDB为用户和管理员提供了对访问权限和权限的细粒度控制。Amazon DynamoDB与IAM服务集成，使用策略提供对权限的强大控制。您可以创建一个或多个允许或拒绝对特定表执行特定操作的策略。也可以使用条件限制对单个项或属性的访问。

* 所有操作必须首先作为有效的用户或用户会话进行身份验证。需要从Amazon DynamoDB读写的应用程序需要获得一组临时或永久的访问控制密钥。虽然这些密钥可以存储在配置文件中，但对于在AWS上运行的应用程序来说，最佳做法是使用IAM Amazon EC2实例配置文件来管理凭据。IAM Amazon EC2实例配置文件或角色允许您避免在配置文件中存储敏感密钥，这些配置文件必须是安全的

* 备注
对于移动应用程序，最佳实践是使用web身份联合和AWS安全令牌服务（AWS STS）的组合来颁发短期后过期的临时密钥

* Amazon DynamoDB还支持细粒度的访问控制，可以限制对表中特定项的访问，甚至限制对项中特定属性的访问，您可能希望限制用户仅访问表中的项，并阻止访问与其他用户关联的项。使用IAM策略中的条件可以限制用户可以执行哪些操作、可以对哪些表执行哪些操作以及可以对哪些属性进行读写


### Amazon DynamoDB Streams

* 许多应用程序的一个常见要求是跟踪最近的更改，然后对更改的记录执行某种处理。Amazon DynamoDB Streams使您很容易获得过去**24小时**内项目修改的列表。例如，您可能需要滚动计算度量并更新仪表板，或者同步两个表，或者记录活动和对审计跟踪的更改。使用Amazon DynamoDB流，这些类型的应用程序变得更容易构建。

* Amazon DynamoDB Streams允许您在不修改原始应用程序的情况下扩展应用程序功能。通过从流中读取活动更改的日志，您可以构建新的集成或支持不属于原始设计的新报告要求。

* 每个项更改都以时间顺序顺序或流缓冲，其他应用程序可以读取这些顺序或流。更改几乎实时地记录到流中，并允许您快速响应或基于修改将一系列事件链接在一起。

* 可以使用AWS管理控制台、命令行界面（CLI）或SDK为Amazon DynamoDB表启用或禁用流。流由流记录组成。每个流记录表示流所属的Amazon DynamoDB表中的单个数据修改。每个流记录被分配一个序列号，反映记录发布到流的顺序。

* 流记录被组织成组，也称为**shards**。每个shard充当多个流记录的容器，并包含有关访问和遍历记录的信息。shard最多可以存活**24**小时，并且随着负载水平的变化，在最终关闭之前可能会被分割一次或多次。

* 要构建从shard读取数据的应用程序，建议使用Amazon DynamoDB Streams kinisis适配器。kinisis客户端库（KCL）简化了处理从流和shard读取记录所需的应用程序逻辑。
